...
for i,a  in enumerate(list_image):
    ...
    if (i + 1) < len(list_image):
        ...
        spitLit = random.randint(8,20)
        image_new_1 = img1[int(image_h1/spitLit):image_h1 , int(image_h1/spitLit):image_h1 ]
        image_new_2 = img2[int(image_h1/spitLit):image_h2 , int(image_h1/spitLit):image_h2 ]

        file_name = str(index) + "_ssasy" + str(i) + str("_.png")
        addh = cv2.hconcat([image_new_1, image_new_2])
        # addh = cv2.vconcat([image_new_1, image_new_2])
        height, width, _ = addh.shape
        M = np.float32([[1, 0, 50], [0, 1, 50]])

        # perform the translation
        img_new_M = cv2.warpAffine(addh, M, (width, height))
        # addh = cv2.hconcat([img1_read, img2_read])
        cv2.imwrite(os.path.join(folder_save, file_name), img_new_M)
        index+=1
