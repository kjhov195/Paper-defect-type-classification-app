...
transform = A.Compose([
    A.Resize(width=224,height=224,p=1),
    A.Transpose(p=0.5),
...
for i,a  in enumerate(list_image):
    ...
    if (i + 1) < len(list_image):
        ...
        # image_new_1 = img1[int(image_h1/8):image_h1 , int(image_h1/8):image_h1 ]
        # image_new_2 = img2[int(image_h1/8):image_h2 , int(image_h1/8):image_h2 ]
        transformed = transform(image=img1)
        transformed2 = transform(image=img2)

        transformed_image = transformed['image']
        transformed_image2 = transformed2['image']
        file_name = str(index) + "_y" + str(i) + str("_.png")
