...
transform = A.Compose([
    ...
    A.ShiftScaleRotate(rotate_limit=15,scale_limit=0.2,p=1),
...
for i,a  in enumerate(list_image):
    ...
    if (i + 1) < len(list_image):
        ...
        image_new1 = img1[0:image_w1  - int(image_w1/10), 0:image_w1 - int(image_w1/10)]
        image_new2 = img2[0:image_w2 - int(image_w2/10), 0:image_w2 - int(image_w2/10)]
        file_name = str(index) + "_xy" + str(i) + str("_.png")
        file_name1 = str(index) + "_xyx" + str(i) + str("_.png")
        file_name2 = str(index) + "_xyz" + str(i) + str("_.png")
        addh = cv2.hconcat([image_new1, image_new2])
        addh2 = cv2.vconcat([img1_read, img2_read])
        transformed = transform(image=addh2)
        #
        transformed_image = transformed['image']
        cv2.imwrite(os.path.join(folder_save, file_name), addh)
        cv2.imwrite(os.path.join(folder_save, file_name1), addh2)
        cv2.imwrite(os.path.join(folder_save, file_name2), transformed_image)
        index+=1
